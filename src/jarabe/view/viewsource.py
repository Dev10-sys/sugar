...
class DocumentButton(RadioToolButton):
    ...
    def __init__(self, file_name, document_path, activity_name, title,
        ...
        if hasattr(self, 'set_icon_widget'):
            self.set_icon_widget(icon)
        icon.set_visible(True)

        box = PaletteMenuBox()
        if hasattr(self, 'props') and hasattr(self.props, 'palette'):
...
class Toolbar(Gtk.Box):
    ...
    def __init__(self, title, bundle_path, document_path, sugar_toolkit_path):
        ...
        if bundle_path is not None and os.path.exists(bundle_path):
            ...
            if hasattr(activity_button, 'set_icon_widget'):
        ...
        if sugar_toolkit_path is not None:
            ...
            icon = Icon(icon_name='computer-xo',
                        ...
                        fill_color=style.COLOR_TRANSPARENT.get_svg(),
                        stroke_color=style.COLOR_WHITE.get_svg())
            if hasattr(sugar_button, 'set_icon_widget'):
...
class SourceDisplay(Gtk.ScrolledWindow):
    ...
    def _show_text_viewer(self):
        ...
        css = "textview { font-family: Monospace; font-size: %dpt; }" % (style.FONT_SIZE / Pango.SCALE)
    ...
    def _show_no_file(self):
        ...
        css_provider.load_from_data(css.encode('utf-8'))
        nofile_box.get_style_context().add_provider(css_provider, Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION)
