...
class ControlPanel(Gtk.Window):
    ...
    def add_alert(self, alert):
        ...
        # Reordering is not natively available like reorder_child, so we handle packing carefully
        # Alerts at the bottom is fine
    ...
    def _set_toolbar(self, toolbar):
        ...
        # Recreate the vbox or use insert_child_after
...
class _SectionIcon(Gtk.Box):
    ...
    def __init__(self, **kwargs):
        ...
        css_provider = Gtk.CssProvider()
        css = "* { color: %s; }" % style.COLOR_WHITE.get_html()
        css_provider.load_from_data(css.encode())
        context = self._label.get_style_context()
        context.add_provider(css_provider, Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION)
